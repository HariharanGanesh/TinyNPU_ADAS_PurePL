...
module tinynpu_hw_test_top (
    ...
    // Clocking & Reset
    // =========================================================================
    // For Option A, we use the raw board clock or a PLL. 
    // Assuming sys_clk is 125MHz on PYNQ-Z2, we run everything off it for this test.
    wire clk = sys_clk;
    wire rst_n = ~sys_rst_n; // FIX: PYNQ-Z2 buttons are Active-High, so we invert it for active-low reset tree
