...
module weight_buffer #(
    ...
    wire [3:0] write_row = write_idx / ARRAY_COLS;
    ...
    genvar r_pk, c_pk;
    generate
        for (r_pk = 0; r_pk < ARRAY_ROWS; r_pk = r_pk + 1) begin : gen_pack_rows
            for (c_pk = 0; c_pk < ARRAY_COLS; c_pk = c_pk + 1) begin : gen_pack_cols
                assign weight_data_flat[(r_pk*ARRAY_COLS + c_pk)*DATA_WIDTH +: DATA_WIDTH] =
                    weight_data[r_pk][c_pk];
            end
        end
    endgenerate
