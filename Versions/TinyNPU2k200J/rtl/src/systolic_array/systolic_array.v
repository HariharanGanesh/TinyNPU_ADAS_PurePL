...
module systolic_array #(
    ...
    //
    // We generate a shared (ARRAY_ROWS + ARRAY_COLS)-stage shift register on
    // the input valid (row 0, col 0) and tap it at the final stage for the output valid.
    // =========================================================================
    reg [ARRAY_ROWS+ARRAY_COLS-1:0] psum_valid_shift;

    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            psum_valid_shift <= 0;
        end else if (array_en) begin
            psum_valid_shift <= {psum_valid_shift[ARRAY_ROWS+ARRAY_COLS-2:0], act_valid_wire[0][0]};
        end
    end
    ...
    generate
        for (c_out = 0; c_out < ARRAY_COLS; c_out = c_out + 1) begin : gen_bottom_outputs
            ...
            assign psum_valid_out[c_out] = psum_valid_shift[ARRAY_ROWS+ARRAY_COLS-1];
        end
    endgenerate
