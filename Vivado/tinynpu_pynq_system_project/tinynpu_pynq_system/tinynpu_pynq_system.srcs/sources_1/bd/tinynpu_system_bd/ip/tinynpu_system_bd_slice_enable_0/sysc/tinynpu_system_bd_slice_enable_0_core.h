...
#ifndef _tinynpu_system_bd_slice_enable_0_core_h_
#define _tinynpu_system_bd_slice_enable_0_core_h_

#include <systemc.h>
#include "properties.h"


class tinynpu_system_bd_slice_enable_0_core : public sc_module 
{
    public: 
       tinynpu_system_bd_slice_enable_0_core (sc_core::sc_module_name nm, const xsc::common_cpp::properties& props)
            : sc_module(nm)
              , Din    ( "Din" )
              , Dout   ( "Dout" )
    {
        SC_HAS_PROCESS(tinynpu_system_bd_slice_enable_0_core);
        SC_METHOD(get_slice_value);
        sensitive << Din;

    }

    virtual ~tinynpu_system_bd_slice_enable_0_core() = default;
