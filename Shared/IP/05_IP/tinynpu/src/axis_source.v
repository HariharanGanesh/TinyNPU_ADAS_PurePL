...
module axis_source #(
    parameter DATA_WIDTH   = 8,    // Bits per output byte / AXI-Stream TDATA
    parameter NUM_CHANNELS = 8,    // Parallel channels per output FIFO word
    parameter TILE_SIZE    = 64,   // Total bytes per tile (WORDS_PER_TILE * NUM_CHANNELS)
    parameter ADDR_WIDTH   = 10    // Kept for port compatibility only; FIFO has no addr port
) (
    input  wire                               clk,
    input  wire                               rst_n,
            ...
            case (state)
                ...
                ST_SEND: begin
                    if (m_axis_tvalid && m_axis_tready) begin
                        ...
                        if (m_axis_tlast) begin
                        ...
                        end else if (byte_idx == NUM_CHANNELS - 1) begin
                            ...
                            m_axis_tdata <= latch_word[next_byte_idx * DATA_WIDTH +: DATA_WIDTH];
                            m_axis_tvalid <= 1'b1;
                            m_axis_tlast  <= (next_byte_idx == NUM_CHANNELS - 1) &&
                        ...
                        // ---------------------------------------------------------
                        m_axis_tdata  <= latch_word[byte_idx * DATA_WIDTH +: DATA_WIDTH];
                        m_axis_tvalid <= 1'b1;
                        m_axis_tlast  <= (byte_idx == NUM_CHANNELS - 1) &&
                                         (word_cnt  == WORDS_PER_TILE - 1);
                    end
