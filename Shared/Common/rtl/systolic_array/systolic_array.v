        ...
        for (r = 0; r < ARRAY_ROWS; r = r + 1) begin : gen_pe_rows
            for (c = 0; c < ARRAY_COLS; c = c + 1) begin : gen_pe_cols
                ...
                always @(posedge clk) begin
                    if (weight_load) $display("PE(%0d,%0d) weight_load=1 weight_in=%h", r, c, weight_data[r][c]);
                    if (array_en && act_valid_wire[r][c]) $display("PE(%0d,%0d) act_valid=1 act_in=%h", r, c, act_wire[r][c]);
                end
            end
        end
    endgenerate
