...
module npu_controller #(
    ...
    // Main FSM
    // =========================================================================
    wire [15:0] total_beats = csr_input_width * csr_input_height;
        ...
        if (!rst_n || csr_soft_reset) begin
            ...
            case (state)
                STATE_IDLE: begin
                    status_idle <= 1'b1;
                    status_busy <= 1'b0;
                    status_done <= 1'b0; // Must clear status_done!
                    if (csr_start) begin
                        status_idle       <= 1'b0;
                        status_busy       <= 1'b1;
                        status_done       <= 1'b0;
                ...
                STATE_SETUP_WGT: begin
                    if (wgt_buf_load_complete) begin
                        ...
                        compute_cycles      <= 0;
                        total_compute_steps <= csr_kernel_size; // Corrected: An 8x8 systolic array needs 8 cycles to compute a matrix multiply, not 64!
                        act_buf_rd_addr     <= 0;
                        act_buf_rd_en       <= 1'b1;
                        state               <= STATE_COMPUTE;
                    end
                ...
                STATE_DRAIN: begin
                    array_en <= 1'b1;
                    // Wait for exact pipeline latency: Systolic(15) + Requant(3) + Act/Pool(1) = 19 cycles
                    // Since act_valid goes down exactly when entering STATE_DRAIN, 
                    // the last output will hit output_buffer at exactly cycle 19.
                    // We must also handle out_buf_full to stall the drain timer if backpressure occurs.
                    if (!out_buf_full) begin
                        if (drain_cycles < (ARRAY_ROWS + ARRAY_COLS + 10)) begin
                            drain_cycles <= drain_cycles + 1'b1;
                        end else begin
                            array_en            <= 1'b0;
                            dma_transfer_size   <= total_beats;
                            // Only pulse dma_start_store_out for exactly 1 cycle
                            dma_start_store_out <= 1'b1; $display("NPU_CONTROLLER: Asserting dma_start_store_out");
                ...
                STATE_STORE_OUT: begin
                    dma_start_store_out <= 1'b0; // Ensure 1-cycle pulse
                ...
                STATE_DONE: begin
                    status_busy <= 1'b0;
                    status_done <= 1'b1; $display("NPU_CONTROLLER: Reached STATE_DONE");
                    state       <= STATE_IDLE;
                end
