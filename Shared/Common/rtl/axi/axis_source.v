        ...
        if (!rst_n) begin
            ...
            case (state)
                ...
                ST_IDLE: begin
                    ...
                    byte_idx      <= 4'd0;
                    word_cnt      <= 4'd0;
                    if (start_drain) $display("AXIS_SOURCE: start_drain=%b buf_empty=%b", start_drain, buf_empty); if (start_drain && !buf_empty) begin
                        latch_word <= buf_rd_data;   // Capture FIFO head (no pop yet)
                        state      <= ST_SEND; $display("AXIS_SOURCE: Entering ST_SEND");
                    end
                end
